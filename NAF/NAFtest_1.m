...
%     for i = 1:11
%         load([FP_TE_ORI num2str(i) '_testpatches.mat']);
%         extNAFFeature(testimgdata,testpatches,NAFfpos,OVH,FP_TE_FEA,[num2str(i) '_test']);
%     end
    
    % 测试patch的KNN
    load([FP_ROOT 'NAFrnode.mat']);
    for x = 2:11
        load([FP_TR_ORI num2str(x) '_trainpatches.mat']);
        load([FP_TE_ORI num2str(x) '_testpatches.mat']);
        load([FP_TE_FEA num2str(x) '_testFea.mat']);
